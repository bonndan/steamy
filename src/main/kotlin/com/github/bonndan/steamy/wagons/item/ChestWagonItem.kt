...
class ChestWagonItem(props: Properties) : Item(props) {
    ...
    override fun use(world: Level, player: Player, hand: InteractionHand): InteractionResult {
        ...
        if (hitResult.type == HitResult.Type.BLOCK) {
            val entity = getEntity(world, itemstack, hitResult, player) ?: return InteractionResult.FAIL
    ...
    private fun getEntity(world: Level, stack: ItemStack, hitResult: BlockHitResult, player: Player): ChestWagonEntity? {

        val entityType: EntityType<ChestWagonEntity> = ModEntityTypes.CHEST_WAGON.get()
        val wagon: ChestWagonEntity? = entityType.create(world, EntitySpawnReason.SPAWN_ITEM_USE)
        if (wagon != null) {
            val vec3 = hitResult.getLocation();
            wagon.setInitialPos(vec3.x, vec3.y, vec3.z);
            if (world is ServerLevel) {
                EntityType<ChestWagonEntity>.createDefaultStackConfig<ChestWagonEntity>(world, stack, player).accept(wagon);
            }
        }
